...
var strictCSP;
var cssCache = [];
var cssCacheUsedWithTarget = false;
var isReplayingCssCache = false;
exports.useStrictCSP = function(value) {
    strictCSP = value;
    if (value == false && !cssCacheUsedWithTarget) insertPendingStyles();
    else if (!cssCache) cssCache = [];
};

function insertPendingStyles() {
    insertPendingStylesForTarget();
}

/**
 * @param {any} [target]
 */
function insertPendingStylesForTarget(target) {
    var cache = cssCache;
    if (!cache) return;

    // Keep cache when replaying into a specific target so other shadow roots
    // can receive the same shared styles later.
    if (!target) {
        cssCache = null;
    } else {
        cssCacheUsedWithTarget = true;
    }

    isReplayingCssCache = true;
    try {
        cache.forEach(function(item) {
            importCssString(item[0], item[1], target);
        });
    } finally {
        isReplayingCssCache = false;
    }
}
...
function importCssString(cssText, id, target) {
    if (typeof document == "undefined")
        return;
    if (cssCache) {
        if (target && !isReplayingCssCache) {
            insertPendingStylesForTarget(target);
