import {
  require_hardwrap
} from "../chunk-SP4OYGKE.js";
import {
  require_multi_select,
  require_multi_select_commands
...
var require_vim = __commonJS({
  "src/keyboard/vim.js"(exports) {
    ...
    var multiSelectCommands = require_multi_select_commands();
    var TextModeTokenRe = require_text().Mode.prototype.tokenRe;
    var hardWrap = require_hardwrap().hardWrap;
    require_multi_select();
      ...
      this.addOverlay = function(o) {
        if (!this.$searchHighlight || !this.$searchHighlight.session) {
          ...
          highlight.updateOnChange = function(delta) {
            ...
            if (row == delta.end.row) highlight.cache[row] = void 0;
...
.ace_dialog input {
  ...
  font-family: monospace;
      ...
      CodeMirror.defineExtension("openDialog", function(template, callback, options2) {
        ...
        function close(newVal) {
          if (typeof newVal == "string") {
            ...
            closed = true;
            dialog.remove();
            if (options2.onClose) options2.onClose(dialog);
            var cm = me;
            if (cm.state.vim) {
        ...
        if (inp) {
          if (options2.value) {
            ...
            if (options2.selectValueOnOpen !== false) inp.select();
          }
          if (options2.onInput)
          ...
          if (options2.closeOnBlur !== false) CodeMirror.on(inp, "blur", close);
          inp.focus();
          ...
          if (options2.closeOnBlur !== false) CodeMirror.on(button, "blur", close);
          button.focus();
        }
        return close;
      });
    ...
    var bigWordCharTest = [function(ch) {
      return /\S/.test(ch);
    }];
    var validMarks = ["<", ">"];
    ...
    var actions = {
      ...
      joinLines: function(cm, actionArgs, vim) {
        ...
        if (vim.visualMode) {
          ...
          curEnd = clipCursorToContent(cm, new Pos(
